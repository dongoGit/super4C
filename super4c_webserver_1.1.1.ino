...
<body>
  <div class="container">
    <header>
      <h1>Super4C</h1>
      <p class="subtitle">Control 8 GPIO pins via ESP32</p>
    </header>
...
const char HTML_CURRENT_STATUS[] PROGMEM = R"rawliteral(
    ...
    <div class="status-bar">
      ...
      <div class="status-item"><strong>PSU1: </strong> <span id="PSUCurrent0">0</span></div>
      <div class="status-item"><strong>PSU2: </strong> <span id="PSUCurrent1">0</span></div>
      <div class="status-item"><strong>PSU3: </strong> <span id="PSUCurrent2">0</span></div>
      <div class="status-item"><strong>PSU4: </strong> <span id="PSUCurrent3">0</span></div>
    </div>
...
void drawStaticContent() {
  ...
  u8g2.drawStr(0, 15, "IP:");
  u8g2.drawStr(0, 30, "DC1:       #2:");
  u8g2.drawStr(0, 45, "DC2:       #3:");
  u8g2.drawStr(0, 60, " #1:       #4:");
}
...
//  u8g2.clearBuffer();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    u8g2.drawStr(20, 15, ip.c_str());
  } else {
    String ip = WiFi.softAPIP().toString() + " (AP)";
    u8g2.drawStr(20, 15, ip.c_str());
  }
  
  //current display
  for (int i = 0; i < 2; i++) {
    u8g2.setCursor(30, 30 + i*15);
    u8g2.print(dcPortCurrent[i], 0);
  }
  
  for (int i = 0; i < 4; i++) {
    int x = (i < 1) ? 30 : 90;
    int y = (i < 1) ? 60 : 30 + (i-1)*15;
    u8g2.setCursor(x, y);
    u8g2.print(psuCurrent[i], 0);
  }
